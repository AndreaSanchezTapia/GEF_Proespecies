...
wip <- "output/p3/p3_NEW_8JUN_IBGE.csv"
wip3 <- read_sf(wip,
        options = c("X_POSSIBLE_NAMES=long_consolidada", "Y_POSSIBLE_NAMES=lat_consolidada"))
raster_veg <- raster::raster("http://www.dpi.inpe.br/amb_data/Brasil/vegetacao_ibge1992_br.asc")
veg <- raster::extract(raster_veg, wip3)
tibble(wip3, IBGE = veg) %>%
  select(-geometry) %>%
  arrange(especie, Nome_coletor_padronizado, numero_coletor, ano_coleta) %>% write_csv("p3_NEW_8JUN_IBGE_veg.csv")
table(veg)
